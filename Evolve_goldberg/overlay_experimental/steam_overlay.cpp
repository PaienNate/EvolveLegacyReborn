...
void Steam_Overlay::FriendConnect(Friend _friend)
{
    ...
    if (id != 0)
    {
        ...
        item.window_title = std::move(_friend.name() + u8" 正在游玩(is playing) " + std::to_string(_friend.appid()));
...
void Steam_Overlay::AddInviteNotification(std::pair<const Friend, friend_window_state>& wnd_state)
{
    ...
    if (id != 0)
    {
        ...
        notif.frd = &wnd_state;
        notif.message = wnd_state.first.name() + u8"邀请您加入他们的游戏(invited you to join a game)";
        notif.start_time = std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now().time_since_epoch());
        notifications.emplace_back(notif);
    }
    else
...
void Steam_Overlay::BuildContextMenu(Friend const& frd, friend_window_state& state)
{
    if (ImGui::BeginPopupContextItem("Friends_ContextMenu", 1))
    {
        ...
        {
            if (i_have_lobby && ImGui::Button(u8"邀请(invite)###PopupInvite"))
            {
                state.window_state |= window_state_invite;
                has_friend_action.push(frd);
                close_popup = true;
            }
            if (state.joinable && ImGui::Button(u8"加入(join)###PopupJoin"))
            {
...
void Steam_Overlay::BuildFriendWindow(Friend const& frd, friend_window_state& state)
{
    ...
    if (ImGui::Begin((state.window_title + friend_window_id).c_str(), &show))
    {
        ...
        {
            ImGui::LabelText("##label", u8"%s正在邀请您一起畅快♂联机.（is inviting you)", frd.name().c_str());
            ImGui::SameLine();
            if (ImGui::Button(u8"同意(Accept)"))
            {
                state.window_state |= window_state_join;
                this->has_friend_action.push(frd);
            }
            ImGui::SameLine();
            if (ImGui::Button(u8"拒绝(Reject)"))
            {
        ...
        wnd_width -= ImGui::CalcTextSize(u8"走你").x + style.FramePadding.x * 2 + style.ItemSpacing.x + 1;
...
std::string get_full_program_pat()
{
...
void Steam_Overlay::OverlayProc()
{
    ...
    if (show_overlay)
    {
        ...
        {
            ImGui::LabelText("##label", u8"用户名(username): %s(%llu) 正在游玩(is playing) %u",
                settings->get_local_name(),
                settings->get_local_steam_id().ConvertToUint64(),
                settings->get_local_game_id().AppID());
            ImGui::SameLine();
            Base_Hook* hook = Renderer_Detector::Inst().get_renderer();
            ImGui::LabelText("##label", u8"渲染方式(render): %s", (hook == nullptr ? "Unknown" : hook->get_lib_name()));

            ImGui::Spacing();

            ImGui::LabelText("##label", u8"好友列表(Friend List)");
            ...
            {
                ImGui::LabelText("##label", u8"暂时没有搜到。(NO FRIEND NOW)");
            }
        }
        ImGui::End();
...
void Steam_Overlay::Callback(Common_Message *msg)
{
    ...
    if (msg->has_steam_messages())
    {
        ...
        if (friend_info != friends.end())
        {
            ...
            AddMessageNotification(friend_info->first.name() + u8" 说(says): " + steam_message.message());
