...
void Steam_Overlay::FriendConnect(Friend _friend)
{
    ...
    if (id != 0)
    {
        ...
        item.window_title = std::move(_friend.name() + u8" playing/正在游玩 " + std::to_string(_friend.appid()));
...
void Steam_Overlay::BuildContextMenu(Friend const& frd, friend_window_state& state)
{
    if (ImGui::BeginPopupContextItem("Friends_ContextMenu", 1))
    {
        ...
        {
            if (i_have_lobby && ImGui::Button(u8"Invite/邀请###PopupInvite"))
            {
                state.window_state |= window_state_invite;
                has_friend_action.push(frd);
                close_popup = true;
            }
            if (state.joinable && ImGui::Button(u8"Join/加入###PopupJoin"))
            {
...
void Steam_Overlay::BuildFriendWindow(Friend const& frd, friend_window_state& state)
{
    ...
    if (ImGui::Begin((state.window_title + friend_window_id).c_str(), &show))
    {
        ...
        wnd_width -= ImGui::CalcTextSize(u8"Send/发送").x + style.FramePadding.x * 2 + style.ItemSpacing.x + 1;
...
void Steam_Overlay::OverlayProc()
{
    ...
    if (show_overlay)
    {
        ...
        if (ImGui::Begin(u8"Goldberg SteamOverlay-Evolve Crack credit: @Nemirtingas, @schmogmog, @nemerod, @kiagam, @pinenut", &show, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoBringToFrontOnFocus))
        {
            ...
            {
            ImGui::LabelText("##label", u8"Username(用户名）: %s(%llu)（正在游玩） playing %u(EVOLVE REBORN)",
            ...
            {
                ImGui::LabelText("##label", u8"Username(用户名）: %s(%llu)（正在游玩） playing %u(OTHER GAME)",
                    settings->get_local_name(),
                    settings->get_local_steam_id().ConvertToUint64(),
                    settings->get_local_game_id().AppID());
            }
            ...
            ImGui::LabelText("##label", u8"Renderer(渲染方式): %s", (hook == nullptr ? "Unknown" : hook->get_lib_name()));

            ImGui::Spacing();

            ImGui::LabelText("##label", u8"Friends(特别提醒：请使用英文输入法，中文输入法会导致游戏崩溃！！)");
